...
- **Защита CSRF**: `protect_from_forgery with: :exception` для HTML-форм;  
  `:null_session` для JSON API. JSON-клиенты обязаны передавать `X-CSRF-Token`.
...
- **Секреты не уходят в браузер**: Учётные данные Supabase хранятся только в  
  переменных окружения сервера и никогда не попадают в HTML или JavaScript.
