...
Директория `packages/` содержит документацию для каждого функционального модуля.
Код реализации находится в корневом `app/` по стандартным соглашениям Rails:

```
packages/
├── start-frt/base/   # Гостевая и аутентифицированная стартовые страницы (документация)
└── auth-frt/base/    # UI аутентификации — вход / регистрация (документация)
```
